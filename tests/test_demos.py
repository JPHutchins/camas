...
def test_demo_sequential_3_steps() -> None:
	task = Sequential(
		lorem_task(0.5, "build", ("resolving deps", "compiling 42 modules", "compiled 42 modules")),
		lorem_task(
			0.8,
			"test",
...
def test_demo_nested_parallel_in_sequential() -> None:
	task = Sequential(
		Parallel(sleep_task(0.5, "format"), sleep_task(0.6, "lint")),
...
def test_demo_nested_sequential_in_parallel() -> None:
	task = Parallel(
		Sequential(
			...
			lorem_task(0.6, "test-lib", ("running tests", "42 tests run", "lib tests passed")),
		),
		Sequential(
	...
	task = Sequential(
		Parallel(
			Sequential(
				...
				lorem_task(0.4, "link", ("resolving symbols", "merging objects", "linking...")),
			),
			lorem_task(0.5, "lint", ("reading config", "walking AST", "checking style...")),
		),
		Parallel(
			lorem_task(0.6, "unit-tests", ("collecting", "running 42 tests", "42 tests passed")),
			lorem_task(
				0.8,
				"integration",
...
def test_demo_matrix_2d_with_nesting() -> None:
	task = Sequential(
		...
		Parallel(
			...
			lorem_task(0.4, "integ {DB}/{OPT}", ("setup", "scenario 1", "scenario 2", "passed")),
		),
		name="ci",
		matrix={"DB": ("sqlite", "postgres"), "OPT": ("debug", "release")},
	)
