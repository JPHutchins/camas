...
def _camas_in(cwd: Path, *args: str) -> subprocess.CompletedProcess[str]:
	return subprocess.run(
		[sys.executable, "-m", "camas", *args],
		cwd=cwd,
		capture_output=True,
...
def test_effects_listing_includes_user_and_builtin() -> None:
	...
	assert "FileLog" in r.stdout, r.stdout
...
def test_filelog_writes_per_task_files(tmp_path: Path) -> None:
	...
	logs = tmp_path / "logs"
	assert logs.is_dir()
	assert (logs / "fast.log").read_text().splitlines() == ["fast 0", "fast 1"]
	assert (logs / "slow.log").read_text().splitlines() == ["slow 0", "slow 1", "slow 2"]
	assert (logs / "done.log").read_text().strip() == "done"


def test_filelog_writes_per_matrix_binding(tmp_path: Path) -> None:
	...
	assert r.returncode == 0, r.stderr
	logs = tmp_path / "logs"
	files = sorted(p.name for p in logs.iterdir())
	# Two leaves (one per binding); each filename encodes the binding.
	assert len(files) == 2, files
	assert any("STAGE=compile" in name for name in files), files
	assert any("STAGE=link" in name for name in files), files
	# Content is the substituted leaf output.
	contents = "\n".join((logs / f).read_text() for f in files)
	assert "built compile" in contents
	assert "built link" in contents
