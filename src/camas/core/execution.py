...
async def run_pipe(stages: tuple[TaskNode, ...], ctx: RunContext) -> tuple[TaskResult, ...]:
	...
	at once, or a full pipe deadlocks its writer. The ``os.pipe()`` fd wiring is exercised on
	Windows by the wheels suite's run of the pipe tests. Whatever fails — a cancel, a spawn
	error, a reader overflow, a landed interrupt — the readers are cancelled and every child is
	killed and awaited before the failure propagates, so no transport outlives the loop.
	"""
	...
	try:
		for stage in leaves:
			...
			if ctx.interrupts.count:
				...
				for remaining in leaves[leaves.index(stage) :]:
					remaining_index = ctx.index_map[id(remaining)]
					await ctx.dispatch(
						remaining_index,
						CompletedEvent(remaining, remaining_index, stopped, datetime.now()),
					)
			...
			if proc.stderr is not None:
				readers.append(asyncio.create_task(read_into(leaf_index, proc.stderr)))
			if is_last and proc.stdout is not None:  # pragma: no branch
				readers.append(asyncio.create_task(read_into(leaf_index, proc.stdout)))
